...
/**
	// Frame type list.
	// Numeric identities are generated by the following script to
	// better control over their values and to compensate for gcc limitations.
	...
	// #!syntax/python
		def inc(line):
			...
			if ip[1].strip().isdigit():
				end = ['"', ' ' + str(v)] + ip[2:] # Overwrite existing.
			else:
				end = ['"', ' ' + str(v) + ip[0]] + ip[1:]
			prefix = '"'.join(parts[:4])
			suffix = ','.join(end)
		...
		import sys
		sys.stdout.writelines(map(inc, sys.stdin.readlines()))
*/
#define STF_EVENTS(EVENT_TYPE) \
	EVENT_TYPE("!", "?", 2162751, message, protocol) \
	EVENT_TYPE("!", "&", 2162726, reference) \
	EVENT_TYPE(">", "<", 4063292, transaction, failed) \
	EVENT_TYPE("<", ">", 3932222, transaction, executed) \
	EVENT_TYPE("-", ">", 2949182, transaction, started) \
	EVENT_TYPE("-", "-", 2949165, transaction, event) \
	EVENT_TYPE("<", "-", 3932205, transaction, stopped) \
	EVENT_TYPE("!", "#", 2162723, message, application) \
	EVENT_TYPE("!", "*", 2162730, message, framework) \
	EVENT_TYPE("!", "~", 2162814, message, trace) \
	EVENT_TYPE("!", "%", 2162725, message, administrative) \
	EVENT_TYPE("!", ">", 2162750, message, entity) \
	EVENT_TYPE("+", "=", 2818109, elements, inserted) \
	EVENT_TYPE("-", "=", 2949181, elements, deleted) \
	EVENT_TYPE("Δ", "=", 60031037, elements, delta) \
	EVENT_TYPE("<", "=", 3932221, elements, reverted) \
	EVENT_TYPE("=", "=", 3997757, elements, committed) \
	EVENT_TYPE("+", ".", 2818094, resource, initialized) \
	EVENT_TYPE("-", ".", 2949166, resource, deleted) \
	EVENT_TYPE("±", ".", 11599918, resource, relocated) \
	EVENT_TYPE("*", ".", 2752558, resource, replicated) \
	EVENT_TYPE("&", ".", 2490414, resource, referenced) \
	EVENT_TYPE("=", ".", 3997742, resource, rewritten) \
	EVENT_TYPE("Δ", ".", 60031022, resource, delta) \
	EVENT_TYPE("^", "*", 6160426, archive, extraction, replicated) \
	EVENT_TYPE("^", "+", 6160427, archive, extraction, replicated, merge) \
	EVENT_TYPE("√", "*", 572129322, archive, delta, replicated, into) \
	EVENT_TYPE("√", "+", 572129323, archive, delta, merged, into) \
	EVENT_TYPE("√", "-", 572129325, archive, delta, deleted) \
	EVENT_TYPE("√", "&", 572129318, archive, delta, referenced) \
	EVENT_TYPE("↑", ".", 563150894, resource, transmitted) \
	EVENT_TYPE("↓", ".", 563281966, resource, received) \
	EVENT_TYPE("↑", ":", 563150906, data, transmitted) \
	EVENT_TYPE("↓", ":", 563281978, data, received) \
	EVENT_TYPE("↓", "↑", 563290513, data, transferred)
...
_stf_event_type_code(const char *f, size_t fs, const char *l, size_t ls)
{
	uint32_t cp[2];
	utf8_identify_codepoint(&cp[0], (uint8_t *) f, fs);
	utf8_identify_codepoint(&cp[1], (uint8_t *) l, ls);
	return(cp[0] << 16 | cp[1]);
}
#define STF_EVENT_TYPE_CODE(F, L) _stf_event_type_code(F, sizeof(F)-1, L, sizeof(L)-1)
